...
function extractTicketNumber(provider, text) {
    ...
    if (cleanProvider === "icon") {
        ...
        //   "No Ticket: CS-123456"
        //   "(NoTicket RENEXRH3)"
        const match = text.match(/(?:No\s+Ticket|NoTicket)\s*:?\s*\(??\s*([A-Z0-9][-A-Z0-9]{4,})/i);
        return match ? match[1].toUpperCase() : null;
    }
    ...
    const genericMatch = text.match(/\b(TKT-\d+|INC\d+|IN\d+|CS-\d+|ID-\d+|FIB-\d+|[A-Z]{3}[A-Z0-9]{5}|\d{9})\b/i);
